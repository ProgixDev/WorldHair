...
/**
 * Materializes the Supabase variant into src/ and test/, removing the
 * MongoDB variant's exclusive files/dependencies/env vars first. See
 * `_variants/supabase/manifest.json` and server/README.md — in particular,
 * this variant does NOT implement register/login/refresh/logout: the
 * mobile/web client calls Supabase's own Auth directly for those.
 *
 * Deliberately not shared with setup-mongodb.ts: two scripts each doing
...
const THIS_VARIANT = "supabase";
const OTHER_VARIANT = "mongodb";
